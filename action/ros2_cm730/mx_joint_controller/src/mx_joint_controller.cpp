    ...
    [ = ](JointCommand::SharedPtr cmd) {
      ...
      std::transform(
        ...
        [&](std::string const & name) -> int {
          ...
          if (iter == jointNames.end()) {
            RCLCPP_WARN(get_logger(), "Unknown joint: %s", name.c_str());
            return -1;
          }
      ...
      std::transform(
        ...
        [ = ](float value) -> uint8_t {
          ...
          if (v > 254) {
            RCLCPP_WARN(get_logger(), "P gain too high: %s", std::to_string(value).c_str());
            v = 254;
          }
          return v;
        });
      ...
      std::transform(
        ...
        [ = ](float value) -> uint8_t {
          ...
          if (v > 254) {
            RCLCPP_WARN(get_logger(), "I gain too high: %s", std::to_string(value).c_str());
            v = 254;
          }
          return v;
        });
      ...
      std::transform(
        ...
        [ = ](float value) -> uint8_t {
          ...
          if (v > 254) {
            RCLCPP_WARN(get_logger(), "D gain too high: %s", std::to_string(value).c_str());
            v = 254;
          }
          return v;
        });
